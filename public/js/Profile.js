$(document).ready(function(){
  ...
  for(var x = 0;x<userArray.length; x++){
    if(userArray[x].username === loggedInUser){
      for(var y = 0; y<userArray[x].games.length; y++){
        ...
        $('#newrow'+y).append('<td class="title">'+userArray[x].games[y].gameName+'</td>');
        $('#newrow'+y).append('<td class="nop">'+userArray[x].games[y].numberOfPlayers+'</td>');
        $('#newrow'+y).append('<td class="status">'+userArray[x].games[y].gameStatus+'</td>');
        $('#newrow'+y).append('<td class="condition">'+userArray[x].games[y].gameCondition+'</td>');
  ...
  $('#submitGamebtn').click(function(e){
    ...
    console.log(number);
    userArray[number].games.push(new game(newGameTitle, newGameStatus, newGameCondition, newMinGameNumberOfPlayers+"-"+newMaxGameNumberOfPlayers));
    console.log(userArray[number]);
    $('#tablebody').append(render(newGameTitle, newGameStatus, newGameCondition, newMinGameNumberOfPlayers +"-"+ newMaxGameNumberOfPlayers));
    saveUsers();
    $('#gamesTitle').val("");
